...
def main():
    ...
    if((target_path/"records_w_diag.pkl").exists()):
        ...
        for id,row in tqdm(df.iterrows(),total=len(df)):
            ...
            hosp_diag, hosp_hadm_id =get_diagnosis_hosp(row["patient_id"], row["ecg_time"])
            tmp["hosp_diag"] = hosp_diag
        ...
        df_full["hosp_diag"]=df_full["hosp_diag"].apply(lambda x: [] if x is None else x)
    ...
    if((target_path/"records_w_diag_icd10.pkl").exists()):
        ...
        df_full["hosp_diag"]=df_full["hosp_diag"].apply(lambda x: [icd_mapping[y] for y in x])
        df_full["hosp_diag"]=df_full["hosp_diag"].apply(lambda x: list(set([y for y in x if (y!="NoDx" and y!=None)])))
        ...
        df_full["ed_or_hosp_diag"]=df_full.apply(lambda row: list(set(row["hosp_diag"]+row["ed_diag_hosp"])),axis=1)
        ...
        df_full.to_pickle(target_path/"records_w_diag_icd10.pkl")
